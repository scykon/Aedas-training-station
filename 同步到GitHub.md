...
## 零、当前实际状态（2026-09-15 更新，以本节为准）

**仓库已经上线了**，走的是「网页手动上传」这条路线（本文档的路线 A）。

- 仓库地址：**https://github.com/scykon/Aedas-training-station**
- 注意：仓库名是 `Aedas-training-station`，**不是**本文后面几节举例用的 `aedas-plan-growth`（后面那些是通用示例，命令里的仓库名以实际为准）
- 文件已通过 GitHub 网页上传完成
...
| 检查项 | 结果 |
|---|---|
| Git | **已安装**：`C:\Users\quxz\AppData\Local\Programs\Git`（版本 2.55.0）。装完必须**重开终端** PATH 才生效；本会话里是用临时 PATH 调用的 |
| 本地仓库 | **已初始化**：`aedas-plan-growth\.git`，分支 `main`，提交 `a94f4d4`（作者 `scykon <scykon@users.noreply.github.com>`），已配 `core.quotepath=false` |
| 远端关联 | 已设置 `origin = https://github.com/scykon/Aedas-training-station.git` |
| 认证 | **尚未打通** —— 这是唯一没做完的一环，详见下面的说明 |
| GitHub CLI（`gh`） | 未安装（安装过程卡在 MSI 阶段，已终止并清理） |
| SSH 密钥 | 没有 |

**关于认证那一步为什么没成**

Git for Windows 自带的凭据助手 GCM（2.9.0）在当前受限执行环境里无法被 git 正常调起，报 `Failed to locate 'git.exe' executable on the path`（直接单独运行它反而正常）。这属于环境限制，不是你账号或仓库的问题。**在你自己开的普通终端里手动执行推送时，GCM 通常会正常弹窗**，所以这条路本身没问题。

**因此：以后想用命令行同步，第一次推送时请你自己开一个普通 PowerShell 窗口执行**（不要依赖自动化环境）：

```powershell
cd "C:\Users\quxz\WorkBuddy\2026-09-15-11-10-34\aedas-plan-growth"
git fetch origin
git reset --hard origin/main      # 让本地历史与网页上传的版本对齐
git push
```

> `git reset --hard` 会丢弃本地那个孤立的初始提交 —— 因为网页上传已经生成了远端自己的历史，两者无共同祖先，直接 push 会被拒绝。如果你不打算用命令行，跳过整段即可，继续用网页上传也没问题。

---

## 一、本机的初始现状（历史记录，已被上一节取代）

| 检查项 | 结果 | 影响 |
|---|---|---|
| Git | 当时**未安装** | 想用命令行就必须先装 |
| GitHub CLI（`gh`） | 未安装 | 建仓库得去网页操作 |
| SSH 密钥 | 没有 | 首次推送需要走 Token 或新建密钥 |
...
2. **单文件 100MB 硬限制。** 以后手绘图如果是 iPad 导出的原图（可能十几 MB），长期累积会让仓库迅速膨胀。建议导出时压到 1MB 以内再放 `uploads`；真需要保留原图，就放到网盘，仓库里只放压缩版。
